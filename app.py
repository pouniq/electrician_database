...
def dashboard():
    with get_connection() as conn:
        ...
        open_jobs = conn.execute("SELECT COUNT(*) FROM jobs WHERE status != 'Done'").fetchone()[0]
        unpaid_total = conn.execute("SELECT COALESCE(SUM(amount), 0) FROM payments WHERE paid = 0").fetchone()[0]

        recent_jobs = conn.execute("""
            SELECT jobs.*, customers.name || ' ' || customers.last_name AS customer_name
            FROM jobs
...
def customers():
    with get_connection() as conn:
        rows = conn.execute("SELECT * FROM customers ORDER BY name, last_name").fetchall()
...
def add_customer():
    if request.method == "POST":
        with get_connection() as conn:
            conn.execute("""
                INSERT INTO customers (name, last_name, phone, address, notes)
                VALUES (?, ?, ?, ?, ?)
            """, (
...
def add_job():
    if request.method == "POST":
        with get_connection() as conn:
            conn.execute("""
                INSERT INTO jobs
                (customer_id, title, description, status, job_date, address, labor_pay)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                ...
                request.form.get("address", "").strip(),
                float(request.form.get("labor_pay") or 0),
            ))
...
def add_material():
    if request.method == "POST":
        with get_connection() as conn:
            conn.execute("""
                INSERT INTO materials (job_id, name, quantity, unit_cost)
                VALUES (?, ?, ?, ?)
            """, (
                request.form["job_id"],
                request.form["name"].strip(),
...
def add_payment():
    if request.method == "POST":
        with get_connection() as conn:
            conn.execute("""
                INSERT INTO payments (job_id, amount, issue_date, paid)
                VALUES (?, ?, ?, ?)
            """, (
                request.form["job_id"],
...
def toggle_paid(payment_id):
    with get_connection() as conn:
        conn.execute("UPDATE payments SET paid = 1 - paid WHERE id = ?", (payment_id,))
    return redirect(url_for("payments"))
